...
| R-020 | Email real P5 aún no fue promovido a producción | Alta | Media | Circuito end-to-end, worker y smoke productivo por digest | Product/Security | Cerrado; producción observada sin fallos |
| R-021 | P1/P2/P3 y el backend Personal-ready requerían promoción productiva | Alta | Media | Promover sólo por digest tras backup, restore y checkpoint productivo | SRE/Security | Cerrado; imagen `a4806315…` saludable, Personal AI aún fail-closed |
...
| R-023 | Adapter Mercado Pago requería validación reversible con vendedor y comprador sandbox sintéticos | Alta | Media | Smoke create/get/cancel/get y limpieza autoritativa; mantener checkout/live apagados | Titular + Billing | Cerrado para sandbox; producción intencionalmente apagada |
...
| R-025 | La preapproval conservaría el 25% después del mes 12 sin repricing automatizado | Alta | Alta | Mantener checkout=false; implementar worker idempotente y reconciliación antes de producción | Billing + Product | Bloqueante productivo identificado |
