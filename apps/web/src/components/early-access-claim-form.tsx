...
import { claimEarlyAccessAction } from '@/server/actions/early-access';
import type { AuthFlowState } from '@/server/action-types';

const INITIAL: AuthFlowState = {};

export function EarlyAccessClaimForm({ token }: { token: string }) {
  const [state, action, pending] = useActionState(claimEarlyAccessAction, INITIAL);
  return (
    <form action={action} className="space-y-3">
      ...
      <button type="submit" disabled={pending || !token} className="w-full rounded-xl bg-accent px-3 py-2.5 text-sm font-medium text-bg disabled:opacity-40">
        {pending ? 'Creando…' : 'Crear mi cuenta Personal'}
      </button>
    </form>
  );
}
